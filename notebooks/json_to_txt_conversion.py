...
for i in range(0, len(json_list)):
    with io.open(json_list[i], encoding='utf-8') as json_values:
        ...
        with io.open(txt_list[i], "w+", encoding='utf-8') as txt_values:
            ...
            # Single page's response located in myRecord['responses'][j]['fullTextAnnotation']['text'], can optionally add the '~ ' page split variable since it's a unique character not in any legal texts
            for j in range(0, len(myRecord['responses'])):
                #page_split = '~ '.encode('utf-8')
